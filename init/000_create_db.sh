...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  ...
  CREATE USER $ACCOUNTSVC_USER WITH NOSUPERUSER NOCREATEDB NOCREATEROLE INHERIT ENCRYPTED PASSWORD '$ACCOUNTSVC_PASSWORD';
  GRANT CONNECT ON DATABASE $YAG_DB TO $ACCOUNTSVC_USER;
  ...
  CREATE USER $AUTHSVC_USER WITH NOSUPERUSER NOCREATEDB NOCREATEROLE INHERIT ENCRYPTED PASSWORD '$AUTHSVC_PASSWORD';
  GRANT CONNECT ON DATABASE $YAG_DB TO $AUTHSVC_USER;
  ...
  CREATE USER $JUKEBOXSVC_USER WITH NOSUPERUSER NOCREATEDB NOCREATEROLE INHERIT ENCRYPTED PASSWORD '$JUKEBOXSVC_PASSWORD';
  GRANT CONNECT ON DATABASE $YAG_DB TO $JUKEBOXSVC_USER;
